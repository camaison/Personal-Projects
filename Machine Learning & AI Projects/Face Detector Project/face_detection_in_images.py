...
grayscale = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  #Converts the coloured image to a gray scale to be easily decipherable by the program
...
cv2.imshow('Face Detector', img) #Displays the image in a program called face detector with rectangles on the detected faces

cv2.waitKey() #Prevent the program from ending Until a button is pressed
